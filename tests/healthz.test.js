...
// // Mock AWS SDK to prevent real S3 interactions during tests
// jest.mock('aws-sdk', () => {
//     const mockS3Instance = {
//         upload: jest.fn().mockReturnValue({
//             promise: jest.fn().mockResolvedValue({ Location: "https://test-bucket.s3.amazonaws.com/testfile.jpg" }),
...
//     return {
//         S3: jest.fn(() => mockS3Instance),
//         config: { update: jest.fn() }, // ✅ Mock AWS.config.update()
//     };
// });
...
//         test('Should return 405 for unsupported methods on /healthz', async () => {
//             const unsupportedMethods = ['post', 'put', 'head', 'options', 'patch'];
//             for (const method of unsupportedMethods) {
//                 const res = await request(app)[method]('/healthz').send({});
//                 expect(res.statusCode).toBe(405);
//             }
...
//     describe('File Upload and Management', () => {
//         beforeEach(async () => {
//             await File.sync({ force: true }); // ✅ Reset DB before each test
//         });
...
//         test('Should return a list of uploaded files', async () => {
//             await File.create({ fileName: 'testfile.jpg', s3Url: 'https://test-bucket.s3.amazonaws.com/testfile.jpg' });
...
describe('Health Check API Tests', () => {
    ...
    beforeEach(async () => {
        await sequelize.sync(); // ✅ Ensures DB is ready before each test
    });
